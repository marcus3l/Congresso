<div class="container">
    <div class=" d-flex" style="display: flex; justify-content: space-between;">
        <div>
            <h2>Cadastro de Sub Área</h2>
        </div>
        <div>
            <a href="<?=base_url()?>restrito/subareas" class="btn btn-danger"><i class="fa fa-chevron-left"></i> voltar</a>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">Para cadastrar uma nova sub área, preencha o formulário abaixo:</div>
                <div class="card-body">
                    <?php
                    if(isset($edit)){
                        $edit = $edit[0];
                    ...
                    ?>
                    <form action="<?=base_url()?>restrito/subareas/salvar" method="POST" id="form" autocomplete="off" enctype="multipart/form-data">
                        <?php
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_area" id="id_area" class="form-control <?=(form_error('id_area')) ? 'is-invalid' : ''; ?>">
                                <option value="" selected disabled>-Selecione-</option>
                                <?php
                                if(isset($areas) && !empty($areas)){
                                    foreach ($areas as $area) {
                                ?>
                                <option value="<?=$area->id?>" <?=set_select('id_area', $area->id, (isset($edit) && $edit->id_area == $area->id ? true : false) )?>><?=$area->nome?></option>
                                <?php
                                    }
                                }
                                ?>
                            </select>
                            <?=form_error('id_area','<small class="text-danger pull-right w-100" style="text-align:right">', '</small>'); ?>
                        </div>
